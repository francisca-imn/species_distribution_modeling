...
data <- read.csv("data/especies/lycalopex_culpaeus.csv", sep = "\t", stringsAsFactors = FALSE) # descargado de GBIF
...
writeRaster(pred, "maxent_lycalopex_culpaeus.tif", format = "GTiff", overwrite = TRUE)  #guardar capa

# comparación con mapa histórico arclim -----------------------------------
arclim_map <- rast("data/arclim/lycalopex_culpaeus_historico.tif")
maxent_map <- rast("maxent_lycalopex_culpaeus.tif")
...
par(mfrow = c(1, 2))
plot(maxent_map, main = "Mapa MAXENT (dismo) histórico")
plot(arclim_map, main = "Mapa ARClim histórico")
...
# Crear un mapa combinado
par(mfrow = c(1, 2))
plot(maxent_future_map, main = "Mapa MAXENT")
plot(arclim_future_map, main = "Mapa ARClim")
